const itinerary = [
  ...
  },
  {
    ...
    route: [
      ...
      "下北澤午餐／古著／咖啡",
      "澀谷十字路口",
      "PARCO／Bic Camera／SHIBUYA SKY 擇一",
      "澀谷或新宿晚餐",
    ],
  ...
  },
  {
    ...
    date: "6/20（六）",
    title: "退房、前往成田、回台灣",
    route: ["退房", "新宿／東京車站寄物或簡單補買", "成田機場", "成田 14:30 起飛", "抵達桃園"],
...
const stays = [
  {
    ...
    dates: "6/13 - 6/16",
    area: "新宿／大久保一帶",
    note: "銜接抵達日與 6/16 前往河口湖；訂房編號與私密入住資訊未放入公開頁。",
  },
  {
    name: "河口湖 Booking｜Yushin／有信旅館",
    dates: "6/16 - 6/17",
    area: "河口湖",
    note: "入住 15:00-18:00，需 18:00 前領鑰匙；這天行程請保留交通緩衝。",
  },
  {
    name: "東京 Airbnb｜後半段",
    dates: "6/17 - 6/20",
    area: "東新宿／新大久保一帶",
    note: "回東京後先放行李，再安排東京鐵塔、六本木或新宿自由活動。",
  },
];
...
function renderDay(index = 0) {
  ...
  timeline.innerHTML = `
    <article class="day-panel">
      ...
      <ol class="route">
        ${item.route.map((stop) => `<li>${escapeHtml(stop)}</li>`).join("")}
      </ol>
